...
Browser AI calls use the existing server implementation. Patient drafts and Google Sheets tokens are stored per Firebase UID. An already-restored login migrates its legacy drafts without changing their contents. Logged-out legacy data whose owner cannot be established is retained under its original keys and is not displayed to the next login; recover it only after confirming the original owner. This separates accounts in the application; local browser storage is not encrypted.
