...
const SUPER_ADMIN_AUTH = { uid: 'superadmin123', email: 'kanata840@gmail.com', email_verified: true };
const NORMAL_USER_AUTH = { uid: 'user123', email: 'normal@test.com', email_verified: true };
const NORMAL_ADMIN_AUTH = { uid: 'admin123', email: 'admin@test.com', email_verified: true };
const UNVERIFIED_USER_AUTH = { uid: 'unverified123', email: 'unverified@test.com', email_verified: false };
...
describe('Firestore Rules', () => {
  ...
  describe('users collection', () => {
    it('allows super admin to create a user', async () => {
      const db = testEnv.authenticatedContext(SUPER_ADMIN_AUTH.uid, SUPER_ADMIN_AUTH).firestore();
    ...
    it('denies normal user from creating a user', async () => {
      const db = testEnv.authenticatedContext(NORMAL_USER_AUTH.uid, NORMAL_USER_AUTH).firestore();
    ...
    it('allows delegated admins to list and delete users', async () => {
      ...
      const db = testEnv.authenticatedContext(NORMAL_ADMIN_AUTH.uid, NORMAL_ADMIN_AUTH).firestore();
